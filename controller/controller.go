...
func (ctrl *ProvisionController) provisionClaimOperation(ctx context.Context, claim *v1.PersistentVolumeClaim) (ProvisioningState, error) {
	...
	item, exists, err := ctrl.volumes.GetByKey(pvName)
	if err == nil && exists {
		pv := item.(*v1.PersistentVolume)
		// PV already exists, check if it's storage matches the claim
		if !claim.Spec.Resources.Requests.Storage().Equal(*pv.Spec.Capacity.Storage()) {
